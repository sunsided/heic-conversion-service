...
impl Convert for ConvertService {
    async fn convert_jpeg(
        ...
        let _ = ConvertService::recode_bytes(&bytes, quality);
...
impl ConvertService {
    fn recode_bytes(bytes: &[u8], quality: u8) -> Result<(), Status> {
        // catch_unwind is required for mozjpeg
        let result = std::panic::catch_unwind(|| {
            ConvertService::recode_bytes_internal(&bytes, quality)
        });
    ...
    fn recode_bytes_internal(bytes: &[u8], quality: u8) -> Result<(), Status> {
